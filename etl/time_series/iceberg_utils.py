...
class IcebergManager:
    def __init__(self, warehouse_path: str, region: str, namespace: str, table_name: str):
    ...
    def write_to_table(self, table: pa.Table, time_col: str = None, unique_cols: list[str] = None) -> bool:
        ...
        try:
            try:
                ...
                logger.info(f"Table {self.full_table_name} already exists. Appending data...")
            except Exception:
                logger.info(f"Table {self.full_table_name} does not exist. Creating it...")
                try:
            ...
            if time_col:
                ...
                if len(existing_data) > 0:
                    ...
                    merged = df_new.merge(df_existing[dedupe_cols].drop_duplicates(), on=dedupe_cols, how='left', indicator=True)
                    df_to_append = merged[merged['_merge'] == 'left_only'].drop(columns=['_merge'])
                    
                    if len(df_to_append) == 0:
                        logger.info(f"All {len(table)} rows already exist. Skipping append.")
                        return True
                        
                    logger.info(f"Identified {len(df_to_append)} new rows out of {len(table)} incoming rows. Appending those...")
                    table = pa.Table.from_pandas(df_to_append, schema=table.schema)
                
            iceberg_table.append(table)
            logger.info(f"Successfully appended {len(table)} rows to {self.full_table_name}")
            return True
        except Exception as e:
    ...
    def read_table(self, row_filter=None) -> pa.Table:
        ...
        try:
            ...
            logger.info(f"Successfully read {len(table_data)} rows from {self.table_name}")
            return table_data
        except Exception as e:
